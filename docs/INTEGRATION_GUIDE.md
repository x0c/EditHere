...
- `Examples/EditHereSample/edithere.project.json` — `projectID`, receiver `baseURL` / `submitPath` / token header hint, build scheme, delivery method, acceptance screens. The checked-in sample uses `http://127.0.0.1:8787` as a placeholder; for on-device Submit, set `baseURL` to your Mac’s LAN address (not on the phone).
